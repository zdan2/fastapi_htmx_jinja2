...
def login(request:Request, email:str=Form(...), password:str=Form(...)):
    with Session(engine) as session:
        ...
        if not user or not verify_password(password,user.password_hash):
            return templates.TemplateResponse('login_form_fragment.html',{'request':request,'error':'Invalid email or password'})
...
def index(request: Request):
    ...
    return templates.TemplateResponse('index.html',{'request':request,'task_list':task_list,'user':user})
...
def delete_task(request:Request, task_id:int):
    user_id=get_user_id(request)
    with Session(engine) as session:
        task=session.exec(select(Todo).where(Todo.user_id==user_id, Todo.id==task_id)).first()
        if not task:
...
def edit_task(request:Request, task_id:int):
    user_id=get_user_id(request)
    with Session(engine) as session:
        task=session.exec(select(Todo).where(Todo.id==task_id,Todo.user_id==user_id)).first()

        if not task:
            raise HTTPException(status_code=404)
    
    return templates.TemplateResponse('task_edit_fragment.html',{'request':request,'task_id':task_id,'todo':task})
...
@app.post("/register")
def register(request: Request, email: str = Form(...), password: str = Form(...),user_name:str=Form(...)):
    ...
    with Session(engine) as session:
        ...
        user = User(email=email, password_hash=hash_password(password),user_name=user_name)
        session.add(user)
        session.commit()
...
def create_admin_if_needed():
    with Session(engine) as session:
        ...
        if not exists:
            u = User(email="admin@example.com", password_hash=hash_password("pass"),user_name='admin')
            session.add(u)
            session.commit()
